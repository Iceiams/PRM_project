...
public class BookDAO {
    ...
    public List<Bookmodel> getSachTop10(String month) {
        ...
        String sSQL = "SELECT Sach.maSach, Sach.tenSach, Sach.giaBia, SUM(HoaDonChiTiet.soLuong) as soluong " +
                "FROM HoaDonChiTiet " +
                "INNER JOIN HoaDon ON HoaDon.maHoaDon = HoaDonChiTiet.maHoaDon " +
                "INNER JOIN Sach ON Sach.maSach = HoaDonChiTiet.maSach " +
                "WHERE strftime('%m', HoaDon.ngayMua) = ? " +
            ...
            try {
                if (cursor.moveToFirst()) {
                    do {
                        ...
                        book.setTenSach(cursor.getString(1));
                        book.setGiaBia(cursor.getDouble(2));
